 ...
 * the same client, so `push` can do nothing a step couldn't. No step is skipped and the first failure
 * stops the run — a step that cannot proceed says why and names its fix (that is each command's own
 * contract), and continuing past it would let a green `preflight` at the end bless a release with a
 * known hole in it.
 *
